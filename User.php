...
class User extends Database
{
    ...
    public function setUser()
    {
        ...
        ('" . $this->name . "','" . $this->email . "', '" . md5($this->password) . "',0); ";
    ...
    {
        $select_query = "SELECT * FROM users WHERE email='" . $this->email . "' 
    ...
    {
        $students_select_query = "SELECT users.id AS userid, users.name AS username,
        users.address AS useraddress, users.status, course.name AS coursename, course.code 
        FROM users LEFT OUTER JOIN course ON users.course_code=course.code WHERE users.role='student';";
